/* ==========================================================================
   PROVIDENTIA FINANCIAL — icones.js
   --------------------------------------------------------------------------
   Ícones do Lucide (https://lucide.dev), licença ISC, uso livre inclusive
   comercial. Os caminhos abaixo são os arquivos originais da biblioteca,
   não são redesenhos meus: a primeira versão do site tinha ícones que eu
   desenhei à mão e ficaram tortos.

   Todos vivem numa grade de 24x24 e usam traço, nunca preenchimento. É por
   isso que eles parecem da mesma família: mesma grade, mesma espessura,
   mesmo acabamento de ponta.

   PARA TROCAR UM ÍCONE
   Vá em https://lucide.dev, procure o que quiser, copie o conteúdo de
   dentro da tag <svg> e cole aqui com um nome novo. Depois rode
   node construir.js.
   ========================================================================== */
...
var ICONES = {
  ...
  /* A lista mental do dia. Entrou no lugar da xícara de café: a xícara
     repetia a ideia de "manhã" que o despertador logo acima já carrega, e
     não dizia nada sobre pensar no resto do dia. */
...
/* Devolve o <svg> pronto.
   classe: a classe CSS ('ico', 'ico-mini', ...)
   Sem aria: todo ícone deste site é decorativo, e o texto ao lado já diz
   o que ele significa. Ícone com rótulo repetido faz o leitor de tela ler
   a mesma coisa duas vezes. */
function icone(nome, classe) {
  var d = ICONES[nome];
  if (!d) { throw new Error('icone inexistente: ' + nome); }
  return '<svg class="' + (classe || 'ico') + '" viewBox="0 0 24 24" aria-hidden="true" focusable="false">' + d + '</svg>';
}

if (typeof module !== 'undefined') { module.exports = { ICONES: ICONES, icone: icone }; }
